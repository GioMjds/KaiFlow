...
export default function Sidebar() {
  ...
  const chatHistory = [
    {
      section: 'Today',
      chats: [
        'Sidebar Design with Motion/Re...',
      ]
    },
    {
      section: '7 Days',
      chats: [
        'Redesign Page with Dark, Refle...',
      ]
    },
    {
      section: '30 Days',
  ...
  return (
      <motion.aside
        ...
        className="fixed left-0 top-0 h-screen bg-[#1a1a1a] border-r border-[#2a2a2a] flex flex-col overflow-hidden z-50"
      >
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-[#2a2a2a] min-h-[68px]">
        {!isSidebarOpen && (
          ...
          >
            <span className="text-3xl font-light">kaiflow</span>
          </motion.div>
        )}
                ...
                {section.chats.map((chat, chatIdx) => (
                  <motion.button
                    ...
                    className={`w-full text-left px-3 py-2.5 rounded-lg transition-colors ${
                      section.section === 'Today' && chatIdx === 0
                  ...
                  >
                    <span className="text-sm line-clamp-1">{chat}</span>
                  </motion.button>
                ))}
              </div>
            </div>
